...
final class DevTimeEngine {
    ...
    static Result lookup(String filmName,
        ...
        addUnique(producerUrls, developerSourceUrl);
        addUnique(producerUrls, filmSourceUrl);
        for (String sourceUrl : producerUrls) {
            if (!looksLikeManufacturer(sourceUrl)) continue;
            try {
                String html = fetch(sourceUrl, 700000);
                List<Row> rows = parseGenericTable(html, sourceUrl, "Produttore");
                Row best = chooseBest(rows, filmName, format, developer, dilution, iso);
                if (best != null) return build(best, targetTemp, format,
                        "Dato trovato nella fonte del produttore associata alla scheda.");
            } catch (Exception ignored) {
            }
        }

        try {
            String url = "https://www.digitaltruth.com/chart/search_text.php?Developer=" +
                    URLEncoder.encode(developer, "UTF-8");
            String html = fetch(url, 900000);
            List<Row> rows = parseDigitaltruth(html, url);
            Row best = chooseBest(rows, filmName, format, developer, dilution, iso);
            if (best != null) return build(best, targetTemp, format,
                    "Nessun dato produttore strutturato trovato; usato Massive Dev Chart.");
        } catch (Exception ignored) {
        }

        try {
            String url = "https://www.digitaltruth.com/devchart.php?Developer=" +
                    URLEncoder.encode(developer, "UTF-8") +
                    "&Film=" + URLEncoder.encode(compactFilmQuery(filmName), "UTF-8") +
                    "&TempUnits=C&TimeUnits=T&mdc=Search";
        ...
        return Result.notFound("Nessuna combinazione esatta trovata online per pellicola + rivelatore + diluizione + ISO + formato.");
    }
