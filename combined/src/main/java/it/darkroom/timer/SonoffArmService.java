...
public final class SonoffArmService extends Service {
    ...
    private volatile int consecutiveEarlyOffs = 0;
    private volatile int consecutivePrematureOffs = 0;
            ...
            if (status == TextToSpeech.SUCCESS && tts != null) {
                ...
                tts.setOnUtteranceProgressListener(new UtteranceProgressListener() {
                    ...
                    @Override public void onDone(String utteranceId) {
                        handleVoiceRepeatFinished(utteranceId);
                    }
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        cancelVoicePrompt();
            ...
            if (mode == MODE_PRINT && !printSequence.isEmpty()) {
                TechnicalLog.add(this, techSessionId, "PIANO DI STAMPA • " + printSequence.summary(widthMs) + " • " + printSequence.detail(widthMs).replace("\n", " • "));
            }
    ...
    private void armInternal() {
        ...
        try {
            ...
            TechnicalLog.add(this, techSessionId, "SETTAGGIO Inching richiesto • " + seconds(currentPulseWidthMs));
        ...
        try {
            ...
            if ("on".equals(sw)) {
                ...
                if (seenOn.compareAndSet(false, true)) {
                    ...
                    lastConfirmedOffBeforeOnAt = 0L;
                    if (mode == MODE_PRINT && !printBaseDone && printSequence != null && !printSequence.isEmpty()) scheduleDodgeCues(estimatedOnAt);
                ...
                long minimumCredibleMs = Math.max(250L, currentPulseWidthMs - 50L);
    ...
    private static String voiceSeconds(int ms) {
        ...
        nextTask = io.schedule(() -> {
            ...
            try {
                ...
                if (TimingMath.isFStop(timingMethod) && testPulsesMs.length == count) {
                    ...
                    configurePulseVerified(currentPulseWidthMs);
                    TechnicalLog.add(this, techSessionId, "COMANDO pulse=on aggiornato • esposizione " + (completed + 1) + "/" + count + " • impulso " + seconds(currentPulseWidthMs) + " • fascia finale " + seconds(TimingMath.physicalTargetAt(testTargetsMs, completed, testStripMethod)));
    ...
    private void scheduleDodgeCues(long observedOnAt) {
        ...
        for (PrintCorrection dodge : printSequence.dodgesForPhase(phase)) {
            ...
            ScheduledFuture<?> f = cueIo.schedule(() -> {
                ...
                dodgeCueFeedback();
                speakOnce("Togli la maschera: " + dodge.safeLabel());
                broadcast(STATE_EXPOSING, msg);
                updateNotification(msg);
    ...
    private void configurePulseVerified(int requestedMs) throws Exception {
