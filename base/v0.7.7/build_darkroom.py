...
def log(msg: str) -> None:
    print(f"[Darkroom v0.7.7] {msg}", flush=True)
...
def preflight(project: Path):
    ...
    if gradle.is_file():
        g = read_text(gradle)
        if not re.search(r"versionCode\s+40\b", g) or not re.search(r"versionName\s+['\"]0\.7\.7['\"]", g):
            fail("app/build.gradle non riporta versionCode 40 / versionName 0.7.7")
    log("Preflight v0.7.7 OK: manifest/versione/requisiti SONOFF invarianti verificati")
