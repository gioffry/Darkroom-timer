...
public final class MainActivity extends Activity implements SonoffDiscovery.Listener {
    ...
    private static final String APP_VERSION = "0.11.0";
    ...
    private void returnPrintToTest() {
        if (armed) return;
        testFromPrint = true;
    ...
    private void adjustTestTime(int direction) {
        if (armed) return;
        if (TimingMath.isFStop(timingMethod)) setTestTime(TimingMath.quarterStop(testWidthMs, direction, 500, testFromPrint ? 36_000_000 : 30_000));
    ...
    private void setTestTime(int ms) {
        if (armed) return;
        testWidthMs = snap(ms, 500, testFromPrint ? 36_000_000 : 30_000);
