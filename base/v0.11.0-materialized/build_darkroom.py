...
import argparse
from v064_icon import materialize_icon
import hashlib
import json
import os
...
def log(msg: str) -> None:
    print(f"[Darkroom v0.11.0] {msg}", flush=True)
...
def preflight(project: Path):
    ...
    if gradle.is_file():
        g = read_text(gradle)
        if not re.search(r"versionCode\s+56\b", g) or not re.search(r"versionName\s+['\"]0\.11\.0['\"]", g):
            fail("app/build.gradle non riporta versionCode 56 / versionName 0.11.0")
    log("Preflight v0.11.0 OK: manifest/versione/requisiti SONOFF invarianti verificati")
...
def make_signing(jdk_keytool: Path, out_dir: Path, provided_ks: str | None,
    ...
    ks = out_dir / "DarkroomTimer-v058-local-signing.p12"
    meta = out_dir / "DarkroomTimer-v058-local-signing.json"
...
def build(args):
    ...
    temp_ctx = tempfile.TemporaryDirectory(prefix="darkroom-v058-")
    temp = Path(temp_ctx.name)
    try:
        project = locate_project(source, temp)
        preflight(project)
        materialize_icon(project)
