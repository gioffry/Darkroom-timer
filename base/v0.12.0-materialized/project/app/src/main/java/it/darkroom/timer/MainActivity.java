...
public final class MainActivity extends Activity implements SonoffDiscovery.Listener {
    ...
    private static final String APP_VERSION = "0.12.0";
    ...
    private LinearLayout buildPrintPanel() {
        ...
        Button chemistrySession = compactButton("CHIMICA SESSIONE · " + it.darkroom.timer.assistant.paper.PaperChemistryStore.shortStatus(this));
    ...
    private void showSplitGradeEditor(final boolean creating) {
        ...
        if (creating) {
            ...
            // Default operativo di riferimento: Fomaspeed Variant 311 RC + testa colore Meopta.
            draft.softYellow = 60;
        ...
        Button syMinus = smallButton("−"); Button syPlus = smallButton("+"); final TextView syValue = text(sy[0] + "Y / 0M", 24, SPLIT_VIVA_MAGENTA, true); syValue.setGravity(Gravity.CENTER);
        ...
        syMinus.setOnClickListener(v -> { sy[0]=Math.max(0,sy[0]-5); syValue.setText(sy[0] + "Y / 0M"); });
        syPlus.setOnClickListener(v -> { sy[0]=Math.min(200,sy[0]+5); syValue.setText(sy[0] + "Y / 0M"); });
        ...
        smPlus.setOnClickListener(v -> { int maxSoft=Math.max(500,printWidthMs-hmsec[0]); sm[0]=Math.min(maxSoft,sm[0]+500); smValue.setText(formatTime(sm[0])); });
        ...
        Button hmMinus = smallButton("−"); Button hmPlus = smallButton("+"); final TextView hmValue = text("0Y / " + hm[0] + "M", 24, SPLIT_VIVA_MAGENTA, true); hmValue.setGravity(Gravity.CENTER);
        ...
        hmMinus.setOnClickListener(v -> { hm[0]=Math.max(0,hm[0]-5); hmValue.setText("0Y / " + hm[0] + "M"); });
        hmPlus.setOnClickListener(v -> { hm[0]=Math.min(200,hm[0]+5); hmValue.setText("0Y / " + hm[0] + "M"); });
        ...
        TextView limit = text("Somma massima delle due esposizioni: " + formatTime(printWidthMs) + " · base operativa", 11, MUTED, true);
        ...
        Button cancel=compactButton("ANNULLA"); cancel.setTextColor(Color.WHITE); cancel.setBackground(roundRect(BUTTON,9,0,0)); cancel.setOnClickListener(v->{dialog.dismiss();showPrintSequenceDialog();}); panel.addView(cancel, margin(lp(-1,dp(48)),0,8,0,0));
    ...
    private LogEntry newEntryFromSession() {
        ...
        if (printAt > 0 && printAt >= testAt) {
            ...
            e.paperChemistrySnapshot = it.darkroom.timer.assistant.paper.PaperChemistryStore.activeSnapshot(this);
            if (testAt > 0) {
    ...
    private void maybeShowTestResultChooser() {
        ...
        showAppChoiceDialog("PROVINO COMPLETATO — SCEGLI LA STRISCIA", choices, which -> {
            ...
            setStatusPresentation("DAL PROVINO — " + formatTime(imported) + ("NESSUNO".equals(chosenFilter) ? "" : " · " + chosenFilter),
                    "Tempo + filtrazione trasferiti insieme alla base di stampa.", GREEN);
        }, "NON ORA");
    }
