...
def log(msg: str) -> None:
    print(f"[Darkroom v0.12.0] {msg}", flush=True)
...
def preflight(project: Path):
    ...
    if gradle.is_file():
        g = read_text(gradle)
        if not re.search(r"versionCode\s+57\b", g) or not re.search(r"versionName\s+['\"]0\.12\.0['\"]", g):
            fail("app/build.gradle non riporta versionCode 57 / versionName 0.12.0")
    log("Preflight v0.12.0 OK: manifest/versione/requisiti SONOFF invarianti verificati")
