...
public final class MyChemistryActivity extends Activity {
    ...
    private void showCatalogForm(JSONObject record){
        ...
        String[] dils=SmartCatalog.dilutions(record,false);if(dils.length==0)dils=SmartCatalog.dilutions(record,true);card.addView(AssistantUi.badge(this,dils.length==0?"DILUIZIONE NON DOCUMENTATA":"DILUIZIONI · "+join(dils),dils.length>0));host.addView(card,AssistantUi.margin(this,-1,-2,0,0,0,8));
